...
from ._exceptions import MixedbreadError, APIResponseValidationError
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from mixedbread import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `mixedbread._streaming` for reference",
        )


class StreamAlreadyConsumed(MixedbreadError):
    """
