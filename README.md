...
The Mixedbread Python library provides convenient access to the Mixedbread REST API from any Python 3.7+
...
response = client.base_status_check()
print(response.name)
```

While you can provide an `api_key` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `BEARER_API_KEY="My API Key"` to your `.env` file
...
async def main() -> None:
    response = await client.base_status_check()
    print(response.name)
...
client.with_options(max_retries=5).base_status_check()
```
...
client.with_options(timeout=5.0).base_status_check()
```
...
You can enable logging by setting the environment variable `MIXEDBREAD_LOG` to `debug`.

```shell
$ export MIXEDBREAD_LOG=debug
```
...
client = response.parse()  # get the object that `base_status_check()` would have returned
print(client.name)
```
